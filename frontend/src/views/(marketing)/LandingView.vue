...
<template>
  <div class="mx-auto max-w-5xl px-4 py-8 md:px-6">
    ...
    <section class="py-16 text-center md:py-24">
      ...
      <h1 class="text-foreground text-4xl font-extrabold tracking-tight md:text-6xl">
        Shorten your links,<br class="hidden sm:block" />
        expand your reach
      </h1>
      <p class="text-muted-foreground mx-auto mt-4 max-w-xl text-lg">
        Create short, memorable links in seconds. Track clicks and manage all your URLs from one
        dashboard.
      </p>
      ...
      <Card>
        ...
        <CardContent>
          <p class="text-muted-foreground text-sm">
            Redirects in under 100ms with Redis-powered caching.
          </p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <p class="text-muted-foreground text-sm">
            URL validation, rate limiting, and JWT authentication.
          </p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <p class="text-muted-foreground text-sm">
            Start free, upgrade when you grow. Plans up to 100,000 URLs.
          </p>
        </CardContent>
      </Card>
    </section>
    ...
    <section class="py-12 text-center">
      <h2 class="text-foreground text-3xl font-bold tracking-tight">Simple pricing</h2>
      ...
      <div v-if="plans.loading" class="text-muted-foreground mt-10 text-sm">Loading plans…</div>
        ...
        >
          <CardHeader class="pb-2">
            ...
            <div class="flex items-baseline gap-1 pt-1">
              ...
              <span v-if="plan.price_cents > 0" class="text-muted-foreground text-sm">/mo</span>
            </div>
          </CardHeader>
          <CardContent class="space-y-1.5">
            <p class="text-muted-foreground text-sm">
              Up to
              <strong class="text-foreground">{{ plan.max_urls.toLocaleString() }}</strong> URLs
            </p>
            <p v-if="plan.features.custom_alias" class="text-muted-foreground text-sm">
              ✓ Custom short codes
            </p>
            <p v-if="plan.features.api_access" class="text-muted-foreground text-sm">
              ✓ API access
            </p>
            <Button
